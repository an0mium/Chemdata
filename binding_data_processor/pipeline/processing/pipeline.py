...
class ProcessingPipeline:
        ...
        try:
            for source in self.config.web_sources:
                try:
                    ...
                    source_compounds = self.community_client.get_compounds(
                        source=source
                    )
    ...
    def _process_social_media(self) -> List[EnhancedCompound]:
        ...
        try:
            ...
            for subreddit in self.config.subreddits:
                try:
                    reddit_compounds = self.web_enrichment.get_reddit_compounds(
                        subreddit=subreddit
                    )
    ...
    def _save_results(
        ...
        try:
            ...
            self.logger.info(
                f"Saved {len(compounds)} compounds to {output_dir}"
            )
