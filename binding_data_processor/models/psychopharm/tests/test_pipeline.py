...
def test_compounds():
    ...
    caffeine.add_receptor_binding("A2A", affinity=0.8, confidence=0.95, activity="antagonist")
    ...
    amphetamine.add_receptor_binding("DAT", affinity=0.05, confidence=0.95, activity="inhibitor")
...
def test_data(tmp_path):
    ...
    data = pd.DataFrame({"name": ["Caffeine", "Amphetamine"], "smiles": ["CN1C=NC2=C1C(=O)N(C(=O)N2C)C", "CC(N)CC1=CC=CC=C1"], "cas_number": ["58-08-2", "300-62-9"]})
...
class TestPipeline:
    ...
    def test_pipeline_stages(self, pipeline, test_compounds):
        ...
        # Test export stage
        result = pipeline.run_export(compounds=test_compounds, output_file=pipeline.config.output_file)
        assert result.success
    ...
    def test_pipeline_checkpointing(self, pipeline, test_data, tmp_path):
        ...
        config = PipelineConfig(input_file=test_data, output_file=tmp_path / "output.tsv", checkpoint_dir=tmp_path / "checkpoints", checkpoint_interval=1)
        ...
        pipeline = Pipeline()
        result = pipeline.recover_from_checkpoint(checkpoint_file=checkpoints[-1], config=config)
        assert result.success
        assert len(result.compounds) == 2
    ...
    def test_pipeline_error_handling(self, pipeline, test_data, tmp_path):
        ...
        config = PipelineConfig(input_file=test_data, output_file=tmp_path / "output.tsv", enrich_sources=["invalid_source"])
...
class TestDataLoader:
    ...
    def test_load_tsv(self, pipeline, test_data):
        ...
        assert result.success
        assert len(result.compounds) == 2
        assert all(isinstance(c, PsychopharmCompound) for c in result.compounds)

    def test_load_json(self, pipeline, tmp_path):
        """Test loading JSON data."""
        # Create test JSON file
        input_file = tmp_path / "input.json"
        data = {"compounds": [{"name": "Caffeine", "smiles": "CN1C=NC2=C1C(=O)N(C(=O)N2C)C", "cas_number": "58-08-2"}]}
...
class TestDataEnricher:
    ...
    def test_chembl_enrichment(self, mock_get, pipeline, test_compounds):
        ...
        mock_response.json.return_value = {"compounds": [{"molecule_chembl_id": "CHEMBL113", "molecule_properties": {"alogp": "0.61", "psa": "58.44"}}]}
    ...
    def test_pubchem_enrichment(self, mock_get, pipeline, test_compounds):
        ...
        mock_response = MagicMock()
        mock_response.json.return_value = {"PC_Compounds": [{"id": {"id": {"cid": 2519}}, "props": [{"urn": {"label": "Molecular Weight"}, "value": {"sval": "194.19"}}]}]}
        mock_get.return_value = mock_response
...
class TestDataExporter:
    ...
    def test_export_tsv(self, pipeline, test_compounds, tmp_path):
        ...
        result = pipeline.exporter.export_tsv(compounds=test_compounds, output_file=output_file, columns=["name", "cas_number", "receptor_profiles"])
        assert result.success
        assert output_file.exists()
        df = pd.read_csv(output_file, sep="\t")
        assert len(df) == 2
    ...
    def test_export_json(self, pipeline, test_compounds, tmp_path):
        ...
        result = pipeline.exporter.export_json(compounds=test_compounds, output_file=output_file)
        assert result.success
        assert output_file.exists()
    ...
    def test_invalid_export(self, pipeline, test_compounds, tmp_path):
        ...
        # Test invalid format
        result = pipeline.exporter.export(compounds=test_compounds, output_file=tmp_path / "export.txt", format="invalid")
        assert not result.success
        assert "invalid format" in str(result.error).lower()
