...
class ActivityPredictor(PredictorBase):
    ...
    def predict(
        ...
        try:
            ...
            self.logger.error(f"Error in activity prediction: {str(e)}")
            return {}
            ...
            for i in range(0, len(compounds), batch_size):
                ...
                for j in range(len(graphs)):
                    ...
                    result["feature_importance"] = {
                        k: v[j] for k, v in importance.items()
                    }
    ...
    def _process_predictions(
        ...
        activities = []
        for i, (act_type, prob, conf) in enumerate(
            zip(self.ACTIVITY_TYPES, preds[activity_slice], confs[activity_slice])
        ):
        ...
        effects = []
        for i, (effect, prob, conf) in enumerate(
            zip(self.PSYCHO_EFFECTS, preds[effects_slice], confs[effects_slice])
        ):
            if conf >= confidence_threshold:
                effects.append(
                    {
        ...
        receptors = []
        for i, (receptor, prob, conf) in enumerate(
            zip(self.RECEPTOR_TYPES, preds[receptor_slice], confs[receptor_slice])
        ):
            if conf >= confidence_threshold:
                receptors.append(
                    {
                ...
                if uncs is not None:
                    receptors[-1]["uncertainty"] = float(
                        uncs[i + n_activities + n_effects]
                    )
