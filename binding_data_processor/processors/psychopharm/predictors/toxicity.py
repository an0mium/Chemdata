...
class ToxicityPredictor(PredictorBase):
                ...
                for organ, toxicities in self.organ_toxicity.items():
                    ...
                    for toxicity in toxicities:
                        ...
                        if model_path.exists():
                            ...
                            self.logger.warning(f"Model not found for {organ}_{toxicity}, initializing new model")
                ...
                for category, concerns in self.safety_concerns.items():
                    ...
                    for concern in concerns:
                        ...
                        if model_path.exists():
                            ...
                            self.logger.warning(f"Model not found for {concern}, initializing new model")
    ...
    def _initialize_models(self) -> Dict:
        ...
        for organ, toxicities in self.organ_toxicity.items():
            organ_type_models = {toxicity: self._initialize_organ_model() for toxicity in toxicities}
            models["organs"][organ] = organ_type_models

        # Initialize safety concern models
        for category, concerns in self.safety_concerns.items():
            category_models = {concern: self._initialize_concern_model() for concern in concerns}
    ...
    def predict(self, compound: CompoundData) -> PredictionResult:
        ...
        try:
            ...
            self.logger.error("Error predicting toxicity", f"Compound {compound.name}: {str(e)}", exc_info=True)
    ...
    def _predict_class(
        ...
        toxicity_class = ToxicityClass(model.classes_[pred_idx]).value
    ...
    def get_prediction_statistics(self) -> pd.DataFrame:
        ...
        stats["organ_confidence"] = self.prediction_history.groupby(["organ", "toxicity_type"])["organ_confidence"].mean()
        ...
        stats["risk_confidence"] = self.prediction_history.groupby(["concern_category", "concern"])["risk_confidence"].mean()
        ...
        for organ, toxicities in organ_data.items():
            for toxicity, severities in toxicities.items():
                if toxicity in self.organ_toxicity.get(organ, []):
                    ...
                    self.logger.debug(f"Organ model for {organ}_{toxicity} training score: {score:.3f}")
        ...
        for category, concerns in concern_data.items():
            for concern, risks in concerns.items():
                if concern in self.safety_concerns.get(category, []):
                    ...
                    self.logger.debug(f"Concern model for {concern} training score: {score:.3f}")
