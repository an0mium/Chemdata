...
class BBBPredictorWebEnriched(BBBPredictorEnhanced):
    ...
    def predict(self, compound: CompoundData) -> PredictionResult:
        ...
        try:
            ...
            result = self._update_prediction_with_web_data(
                result, processed_data, compound
            )
    ...
    def _update_web_prediction_history(
        ...
        self.prediction_history.at[latest_idx, 'chembl_data'] = str(
            web_data.get("chembl", {})
        )
        self.prediction_history.at[latest_idx, 'pubchem_data'] = str(
            web_data.get("pubchem", {})
        )
        self.prediction_history.at[latest_idx, 'swiss_data'] = str(
            web_data.get("swiss", {})
        )
        self.prediction_history.at[latest_idx, 'community_data'] = str(
            web_data.get("community", {})
        )
        self.prediction_history.at[latest_idx, 'social_data'] = str(
            web_data.get("social", {})
        )
        self.prediction_history.at[latest_idx, 'web_mentions'] = str(
            web_data.get("web_search", {})
        )
        self.prediction_history.at[latest_idx, 'mechanism_data'] = str(
            web_data.get("mechanism_data", {})
        )
        self.prediction_history.at[latest_idx, 'pharmacology_data'] = str(
            web_data.get("pharmacology_data", {})
        )
        self.prediction_history.at[latest_idx, 'safety_data'] = str(
            web_data.get("safety_data", {})
        )
        self.prediction_history.at[latest_idx, 'interaction_data'] = str(
