...
from ....models.psychopharm.compound import PsychopharmCompound
...
class ReceptorBindingConfig(PredictorConfig):
    ...
    RECEPTOR_FAMILIES = {
        "serotonin": {"5-HT1A", "5-HT1B", "5-HT1D", "5-HT2A", "5-HT2B", "5-HT2C", "5-HT3", "5-HT4", "5-HT5A", "5-HT6", "5-HT7"},
        "dopamine": {"D1", "D2", "D3", "D4", "D5"},
        "norepinephrine": {"α1A", "α1B", "α1D", "α2A", "α2B", "α2C", "β1", "β2", "β3"},
        "glutamate": {"NMDA", "AMPA", "Kainate", "mGluR1", "mGluR2", "mGluR3", "mGluR4", "mGluR5", "mGluR6", "mGluR7", "mGluR8"},
        "gaba": {"GABA-A", "GABA-B", "GABA-C"},
        "opioid": {"μ", "κ", "δ", "NOP"},
        "cannabinoid": {"CB1", "CB2"},
    ...
    ACTIVITY_TYPES = ["full_agonist", "partial_agonist", "antagonist", "inverse_agonist", "allosteric_modulator", "unknown"]

    def __init__(self, model_dir: Optional[str] = None, receptor_families: Optional[Dict[str, Set[str]]] = None, activity_types: Optional[List[str]] = None, **kwargs):
        """Initialize config.
...
class ReceptorPredictor(WebEnrichedPredictor):
    ...
    def __init__(
        ...
        self.scalers = {feature_type: StandardScaler() for feature_type in self.feature_types}

        # Web enrichment
        web_config = EnrichmentConfig(
            model_dir=Path(model_dir) if model_dir else None,
            cache_dir=Path(cache_dir) / "web" if cache_dir else None,
            use_cache=True,
            n_workers=4,
        )
        self.web_manager = WebEnrichmentManager(config=web_config)
        ...
        self.logger.info("ReceptorPredictor initialized successfully")
    ...
    def _load_affinity_model(self, receptor: str) -> RandomForestRegressor:
        ...
        if self.config.model_dir:
            ...
            try:
                ...
                self.logger.warning(f"Failed to load affinity model for {receptor}: {str(e)}")

        # Create new model if loading fails
        self.logger.debug(f"Creating new affinity model for {receptor}")
        return RandomForestRegressor(n_estimators=100, max_depth=10, random_state=42, n_jobs=-1, verbose=0)
    ...
    def _load_type_model(self, receptor: str) -> GradientBoostingRegressor:
        ...
        if self.config.model_dir:
            ...
            try:
                ...
                self.logger.warning(f"Failed to load type model for {receptor}: {str(e)}")

        # Create new model if loading fails
        self.logger.debug(f"Creating new type model for {receptor}")
        return GradientBoostingRegressor(n_estimators=100, max_depth=5, random_state=42, verbose=0)

    def _extract_features(self, compound: PsychopharmCompound) -> np.ndarray:
    ...
    def _predict_raw(self, features: np.ndarray) -> Tuple[Dict, float]:
        ...
        for receptor in self.config.target_receptors:
            try:
                ...
                affinity_conf = 1.0 - np.std([tree.predict([features])[0] for tree in affinity_model.estimators_])
                ...
                self.logger.error(f"Prediction error for {receptor}: {str(e)}")
                continue
    ...
    def _process_prediction(self, prediction: Dict, confidence: float, compound: PsychopharmCompound) -> PredictionResult:
        ...
        try:
            ...
            return PredictionResult(value=results, confidence=confidence, metadata=metadata)

        except Exception as e:
            self.logger.error(f"Error processing predictions for {compound.name}: {str(e)}")
            return PredictionResult(value={}, confidence=0.0, metadata={"error": str(e)})
    ...
    def _get_web_data(self, compound: PsychopharmCompound) -> Optional[Dict]:
        ...
        try:
            # Get ChEMBL data
            chembl_data = self.web_manager.get_chembl_data(compound.smiles, targets=self.config.target_receptors)
            if chembl_data:
                binding_data["chembl"] = chembl_data

            # Get PubChem data
            pubchem_data = self.web_manager.get_pubchem_data(compound.smiles, targets=self.config.target_receptors)
            if pubchem_data:
                binding_data["pubchem"] = pubchem_data

            # Get patent data
            patent_data = self.web_manager.get_patent_data(compound.smiles, targets=self.config.target_receptors)
            if patent_data:
            ...
            self.logger.error(f"Error getting web data for {compound.name}: {str(e)}")
            return None
        ...
        try:
            ...
            for receptor in self.config.target_receptors:
                ...
                if web_values:
                    ...
                    combined_affinity = (ml_data["affinity"] * ml_weight + np.mean(web_values) * web_weight) / (ml_weight + web_weight)
            ...
            return PredictionResult(value=combined_results, confidence=float(np.mean([r["confidence"] for r in combined_results.values()])), metadata=ml_result.metadata)

        except Exception as e:
            self.logger.error(f"Error combining predictions for {compound.name}: {str(e)}")
            return ml_result
    ...
    def get_prediction_statistics(self) -> pd.DataFrame:
        ...
        stats["affinity_mean"] = self.prediction_history.groupby("receptor")["affinity_value"].mean()
        stats["affinity_std"] = self.prediction_history.groupby("receptor")["affinity_value"].std()

        # Activity type distribution
        stats["activity_dist"] = self.prediction_history.groupby("receptor")["activity_type"].value_counts(normalize=True)

        # Average confidence scores
        stats["affinity_confidence"] = self.prediction_history.groupby("receptor")["affinity_confidence"].mean()
        stats["activity_confidence"] = self.prediction_history.groupby("receptor")["activity_confidence"].mean()
