...
class PsychoactiveConfig(PredictorConfig):
    ...
    EFFECT_CATEGORIES = {
        "cognitive": {
            ...
            "cognitive_suppression",
            "thought_acceleration",
            "thought_deceleration",
            "conceptual_thinking",
            "abstract_thinking",
            "creativity",
            "analysis",
    ...
    def __init__(self, effect_categories: Optional[Dict[str, Set[str]]] = None, activity_types: Optional[List[str]] = None, **kwargs):
        """Initialize config.
...
class PsychoactivePredictor(WebEnrichedPredictor):
        ...
        if self.model_dir:
            try:
                ...
                if class_path.exists():
                    ...
                    self.logger.warning("Class predictor not found, initializing new model")
                ...
                for effect in self.config.effects:
                    ...
                    if model_path.exists():
                        ...
                        self.logger.warning(f"Model not found for {effect}, initializing new model")
                ...
                if model_path.exists():
                    ...
                    self.logger.warning("Activity type model not found, initializing new model")
    ...
    def _initialize_class_model(self) -> RandomForestClassifier:
        ...
        return RandomForestClassifier(n_estimators=100, max_depth=10, random_state=42, n_jobs=-1, verbose=0)
    ...
    def _initialize_activity_model(self) -> RandomForestClassifier:
        ...
        return RandomForestClassifier(n_estimators=100, max_depth=10, random_state=42, verbose=0)
    ...
    def _predict_raw(self, features: np.ndarray) -> Tuple[Dict, float]:
        ...
        try:
            ...
            primary_class = PsychoactiveClass(class_model.classes_[class_idx]).value
            ...
            for effect, model in self.models["effects"].items():
                ...
                intensity_conf = 1.0 - np.std([tree.predict([features])[0] for tree in model.estimators_])
    ...
    def _get_web_effect_data(self, web_data: Dict, effect: str) -> Tuple[List[float], List[float]]:
    ...
    def _combine_effect_predictions(self, ml_data: Dict, web_intensities: List[float], web_confidences: List[float]) -> Dict:
        ...
        combined_intensity = (ml_data["intensity"] * ml_weight + np.mean(web_intensities) * web_weight) / (ml_weight + web_weight)
        ...
        try:
            ...
            for category, effects in self.config.effect_categories.items():
                ...
                for effect in effects:
                    ...
                    # Combine predictions
                    category_results[effect] = self._combine_effect_predictions(ml_data, web_intensities, web_confs)

                if category_results:
                    combined_results["effects"][category] = category_results

            # Calculate overall confidence
            effect_confidences = [effect["confidence"] for category in combined_results["effects"].values() for effect in category.values()]
            confidence = float(np.mean(effect_confidences)) if effect_confidences else 0.0

            return PredictionResult(value=combined_results, confidence=confidence, metadata=ml_result.metadata)
    ...
    def get_prediction_statistics(self) -> pd.DataFrame:
        ...
        # Class distribution
        stats["class_dist"] = self.prediction_history["primary_class"].value_counts(normalize=True)

        # Average class confidence
        stats["class_confidence"] = self.prediction_history.groupby("primary_class")["class_confidence"].mean()

        # Activity type distribution
        stats["activity_dist"] = self.prediction_history["activity_type"].value_counts(normalize=True)

        # Average activity confidence
        stats["activity_confidence"] = self.prediction_history.groupby("activity_type")["activity_confidence"].mean()

        # Effect intensity distribution
        stats["effect_intensity"] = self.prediction_history.groupby(["effect_category", "effect"])["intensity"].mean()

        # Effect confidence distribution
        stats["effect_confidence"] = self.prediction_history.groupby(["effect_category", "effect"])["effect_confidence"].mean()

        return stats

    def retrain(
        self,
        compounds: List[PsychopharmCompound],
        ...
        for category, effects in effect_data.items():
            for effect, intensities in effects.items():
                if effect in self.config.effect_categories.get(category, []):
                    ...
                    self.logger.debug(f"Effect model for {effect} training score: {score:.3f}")
