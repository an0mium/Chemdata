...
class CompoundCompiler:
        ...
        for search in self.queries:
            ...
            for patent in patents:
                try:
                    ...
                    print(f"Error processing patent {patent['patent_number']}: {str(e)}")
                    continue
    ...
    def create_tsv(self, compounds: List[Dict[str, Any]], filename: str):
        ...
        df['activity_data'] = df['mentions'].apply(
            lambda x: '\n'.join(
                f"{m['context']}" 
                for m in x if isinstance(m, dict) and 
                (m.get('has_activity') or m.get('has_binding') or m.get('has_affinity'))
...
async def main():
    ...
    stats = compiler.create_tsv(
        compounds,
        f'receptor_compounds_{timestamp}.tsv'
    )
